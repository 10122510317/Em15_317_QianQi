set_property IOSTANDARD LVCMOS33 [get_ports {led_4bit_tri_o[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {led_4bit_tri_o[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {led_4bit_tri_o[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {led_4bit_tri_o[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports {btns_4bit_tri_i[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {btns_4bit_tri_i[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {btns_4bit_tri_i[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {btns_4bit_tri_i[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports {input_4bit_tri_i[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {input_4bit_tri_i[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {input_4bit_tri_i[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {input_4bit_tri_i[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports {light_4bit_tri_o[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {light_4bit_tri_o[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {light_4bit_tri_o[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {light_4bit_tri_o[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports {sw_4bit_tri_i[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {sw_4bit_tri_i[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {sw_4bit_tri_i[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {sw_4bit_tri_i[0]}]
set_property PACKAGE_PIN D18 [get_ports {led_4bit_tri_o[3]}]
set_property PACKAGE_PIN G14 [get_ports {led_4bit_tri_o[2]}]
set_property PACKAGE_PIN M15 [get_ports {led_4bit_tri_o[1]}]
set_property PACKAGE_PIN M14 [get_ports {led_4bit_tri_o[0]}]
set_property PACKAGE_PIN Y16 [get_ports {btns_4bit_tri_i[3]}]
set_property PACKAGE_PIN V16 [get_ports {btns_4bit_tri_i[2]}]
set_property PACKAGE_PIN P16 [get_ports {btns_4bit_tri_i[1]}]
set_property PACKAGE_PIN R18 [get_ports {btns_4bit_tri_i[0]}]
set_property PACKAGE_PIN W14 [get_ports {input_4bit_tri_i[3]}]
set_property PACKAGE_PIN V15 [get_ports {input_4bit_tri_i[2]}]
set_property PACKAGE_PIN Y14 [get_ports {input_4bit_tri_i[1]}]
set_property PACKAGE_PIN W15 [get_ports {input_4bit_tri_i[0]}]
set_property PACKAGE_PIN V18 [get_ports {light_4bit_tri_o[3]}]
set_property PACKAGE_PIN R14 [get_ports {light_4bit_tri_o[2]}]
set_property PACKAGE_PIN Y17 [get_ports {light_4bit_tri_o[1]}]
set_property PACKAGE_PIN H15 [get_ports {light_4bit_tri_o[0]}]
set_property PACKAGE_PIN T16 [get_ports {sw_4bit_tri_i[3]}]
set_property PACKAGE_PIN W13 [get_ports {sw_4bit_tri_i[2]}]
set_property PACKAGE_PIN P15 [get_ports {sw_4bit_tri_i[1]}]
set_property PACKAGE_PIN G15 [get_ports {sw_4bit_tri_i[0]}]
